...
def prepare_data(file_path, MAX_WORDS, MAX_LEN):
    ...
    for i in range(len(labels)):
        sample_weights[:, i] = y_train[:, i] * class_weights[i] + (1 - y_train[:, i]) * 1.0

    sample_weights_flat = np.mean(sample_weights, axis=1).astype("float32")

    train_ds = (
        tf.data.Dataset.from_tensor_slices((X_train, y_train, sample_weights_flat))
        .shuffle(10000)
        .batch(128)
...
def build_model(MAX_WORDS, MAX_LEN, labels):
    ...
    model.compile(
        optimizer="adam", loss="binary_crossentropy", metrics=[tf.keras.metrics.AUC(name="auc", multi_label=True)]
    )
    return model
    ...
    if current_auc > prod_auc:
        ...
        if logged_dataset_artifact is not None:
            try:
                ...
                print("Warning: Could not load dataset artifact")
    else:
...
def main():
    ...
    model_artifact = wandb.Artifact(name="toxic-comment-multilabel", type="model", metadata=test_results)
